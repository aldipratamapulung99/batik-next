...
export async function ambilSemuaPesan() {
  ...
  return (data || []).map((p) => ({
    ...p,
    nama_produk: p.produk?.nama_produk || null,
    harga_produk: p.produk?.harga ?? null,
  }));
}
...
export async function hapusProduk(id) {
  ...
  // Produk yang sudah pernah dipesan tidak boleh dihapus: tabel pesan masih merujuk ke produk ini
  // dan riwayat penjualan/laporan akan rusak. Sarankan set stok 0 sebagai gantinya.
  const { count, error: errorHitung } = await supabase
    .from('pesan')
    .select('id', { count: 'exact', head: true })
    .eq('produk_id', id);
  ...
  if (count > 0) {
    return {
      sukses: false,
      pesan: `Produk ini sudah pernah dipesan (${count} pesanan) sehingga tidak bisa dihapus. Ubah stoknya menjadi 0 lewat Edit kalau tidak ingin dijual lagi.`,
    };
  }
